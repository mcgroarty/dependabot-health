...
- [ ] Guard against division by zero in `config-coverage.py` when an org has zero non-archived, non-empty repos, and make sure the script still emits a sensible summary/CSV.
- [ ] Fix `p50_open_age_days` in `alert-age.py` so even-sized alert sets report a true median instead of the upper middle value.
